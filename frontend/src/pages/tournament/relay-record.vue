...
const shareTitle = computed(() => {
  const leftName = record.value?.leftTeam?.name
  const rightName = record.value?.rightTeam?.name
...
async function loadRecord() {
  ...
  try {
    ...
    if (!divisionId.value) divisionId.value = lineupData?.divisionId || recordData?.divisionId || ''
    await loadTournamentInfo()
    await loadStageText()
    promptSealReport()
  } catch (error) {
