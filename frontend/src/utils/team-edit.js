...
// 后端约定：仅创建者可改队名/追加队员；不支持增删队伍与删除队员。
 ...
 * rename 为 true 时携带队名；member 非空时携带 addMembers（羽毛球不带号码字段）。
 */
export function buildUpdateTeamPayload({ teamName, rename, member, volleyball }) {
  const payload = {}
