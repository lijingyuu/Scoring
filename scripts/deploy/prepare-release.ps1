...
function Resolve-ProjectRoot {
  ...
  while ($null -ne $current) {
    if (
      ...
      (Test-Path (Join-Path $current "frontend\package.json")) -and
      (Test-Path (Join-Path $current "frontend\src\utils\request.js"))
    ) { return $current }
...
$requestJs = Join-Path $root "frontend\src\utils\request.js"
$requestText = Get-Content $requestJs -Raw
if ($requestText -notmatch "import\.meta\.env\.DEV") {
  throw "frontend/src/utils/request.js must use import.meta.env.DEV to separate dev/prod API base URLs"
}
...
Invoke-Checked "npm.cmd" @("--prefix", "frontend", "run", "build:mp-weixin")

Write-Host "`n== Built domain check =="
$dist = Join-Path $root "frontend\dist\build\mp-weixin"
if (!(Test-Path $dist)) { throw "frontend/dist/build/mp-weixin not found" }
...
Write-Host "Mini program: frontend\dist\build\mp-weixin"