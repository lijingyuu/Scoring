...
因此接力追分的小组排名不能直接套用苏杯五项的“场内大分/场内局/局内小分”模板。后续如需模板，应单独定义，例如胜场、总得失分、直接交手等。
...
个人赛“常用模板一”暂未最终确定。不要用团体赛的场内大分规则解释个人赛。
...
```
胜场 → H2H/多队小循环 → 场内大分差 → 场内局差 → 局内小分差 → 名字序
```
...
校园/大众模板和 FIVB 模板独立于羽毛球：

```
校园/大众：胜场 → 净胜局 → 净胜分 → H2H → 名字序
