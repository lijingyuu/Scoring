...
> 逐项说明本项目**用到了哪些技术/库/框架/设计模式，以及在本项目里具体是怎么落地**——每一条落到真实文件路径与关键代码片段，便于维护与复盘。
> 关联：[[架构.md]] · [[数据库.md]] · [[业务规则.md]] · [[测试策略.md]]
...
- 位置集中在 Service 写方法（门面 `MatchServiceImpl`/`TournamentServiceImpl` 及其协作者 `MatchSettlementService`/`TournamentCreationFactory` 等，见 [[架构.md]]）。
...
**怎么用的**：
- `frontend/src/pages.json`：24 个页面路由 + 2 个 TabBar + `globalStyle.navigationStyle: custom`。
...
- `frontend/src/manifest.json`：`appid`、`vueVersion: "3"`、`mp-weixin.resizable`。
...
`frontend/package.json` 无 pinia/vuex。全局状态是 `frontend/src/store/auth.js` 里的**模块级 `reactive({...})` 单例** + 导出函数操作 state（"模块即 store"）：
...
- **清缓存规范**（项目约定）：本地缓存是当前设备的比赛恢复现场，权限拒绝或资料未完善时只拦截返回、不清缓存；仅在结算成功、重新开始、用户明确放弃时调 `clearMatchState(matchId)`。
