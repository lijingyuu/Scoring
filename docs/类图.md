...
> 实体字段定义以 [[数据库.md]] 为准，本图只展示类结构与关联关系（如 V20 执裁锁字段见数据库 `match_record`）。
...
class TournamentCreationFactory {
  +createTournament(creatorUserId, req)
  -buildPlayers()
...
TournamentServiceImpl --> TournamentCreationFactory
TournamentServiceImpl --> TournamentAccessGuard
TournamentCreationFactory --> TournamentMapper
TournamentCreationFactory --> PlayerMapper
TournamentCreationFactory --> MatchRecordMapper
TournamentCreationFactory --> TournamentRoundRuleMapper
TournamentCreationFactory --> TournamentTeamMemberMapper
TournamentCreationFactory --> BracketEngine
TournamentCreationFactory --> RoundRobinEngine
TournamentAccessGuard --> TournamentMapper
...
class MatchAccessGuard {
  +requireMatchOperator(userId, tournamentId)
}
...
MatchServiceImpl --> MatchSettlementService
MatchServiceImpl --> MatchLockService
MatchServiceImpl --> MatchAccessGuard
MatchSettlementService --> MatchRecordMapper
MatchSettlementService --> TeamMatchItemMapper
MatchSettlementService --> TournamentMapper
MatchSettlementService --> TournamentRuleResolver
MatchLockService --> MatchRecordMapper
MatchRecordMapper --> MatchRecord
...
TournamentController --> TournamentService
TournamentService <|.. TournamentServiceImpl
TournamentServiceImpl --> TournamentRefereeService
TournamentRefereeService --> TournamentRefereeConfigMapper
TournamentRefereeService --> TournamentRefereeGrantMapper
TournamentRefereeService --> UserMapper
MatchService <|.. MatchServiceImpl
...
class TournamentRankingService {
  +getGroupStandings(tournamentId, currentUserId)
  +getRankingConfig(tournamentId, currentUserId)
  +updateRankingConfig(userId, tournamentId, req)
  +updateQualificationOverrides(userId, tournamentId, req)
...
TournamentServiceImpl --> TournamentRankingService
TournamentRankingService --> GroupStandingEngine
GroupStandingEngine --> RankingConfig
TournamentRankingService --> TournamentRankingConfigMapper
TournamentRankingService --> TournamentQualificationOverrideMapper
...
TournamentRankingService --> GroupStandingsVO
```
...
MatchServiceImpl --> MatchReportAssembler
MatchReportAssembler --> MatchReportMetaMapper
