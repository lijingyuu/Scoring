...
| Web 后台 | **Vue 3 + vue-router + Vite** (`web/admin-web/`) | 3.4 / 4.4 / 5.2 | 独立静态站点部署 www.eunomia.cc，无 UI 库，fetch + localStorage 最小实现 |
...
> 除上图主链路外，还有两个独立前端：**`web/admin-web/`（Vue 3 后台，www.eunomia.cc）** 走 nginx 同源反代 `/api/` 调用同一后端；**`web/product-web/`（纯静态产品介绍页，product.eunomia.cc）** 不调 API。详见 [`docs/后台管理.md`](docs/后台管理.md)。
...
| JDK | 17 |
| MySQL | 8.0 |
| Maven | 3.9+（使用系统安装的 Maven，确保 `mvn` 在 PATH） |
| 微信开发者工具 | 最新稳定版 |
...
### Web 后台启动（web/admin-web）

```bash
cd frontend
npm run admin:dev      # → http://localhost:5173，代理 /api → 127.0.0.1:8080
npm run admin:build    # 构建静态产物 → web/admin-web/dist/
```

> 与小程序共用后端和账号体系：小程序微信登录创建的账号需设置过用户名密码（或在后台注册新账号）后才能登录 Web 端。生产部署见 `scripts/deploy/`。
...
| 文档 | 路径 | 用途 |
|------|------|------|
| **AI 入口** | `docs/agent/CLAUDE.md` | 构建命令、文档索引、关键约定 |
| **架构地图** | `docs/架构.md` | 前后端分层、文件职责、路由总览 |
| **后台管理网页** | `docs/后台管理.md` | admin-web（www.eunomia.cc）定位、结构、已完成功能与后续路线 |
| **核心类图** | `docs/类图.md` | 核心领域、赛程生成、记分结算、团体赛、排名引擎类图 |
| **主要用例** | `docs/用例.md` | 核心功能用例、参与者、主流程和验收核对点 |
| **数据字典** | `docs/数据库.md` | 17 张表结构 + 全部枚举映射 |
| **API 契约** | `docs/API接口.md` | 42 个当前有效接口的入参/出参/鉴权 |
| **业务规则** | `docs/业务规则.md` | 赛制流转、计分规则、排名/出线/战报/自由人/队长/换边算法 |
| **设计系统** | `docs/界面设计.md` | 色彩变量、字号阶梯、交互底线 |
| **测试策略** | `docs/测试策略.md` | 测试覆盖说明 + 跑测命令 |
| **技术栈复盘** | `docs/技术栈.md` | 项目用到哪些技术点、在本项目里具体怎么落地 |
| **复盘与答辩** | `docs/面试准备.md` | 项目亮点、踩坑复盘、设计决策答辩稿 |
| **试跑清单** | `docs/实测清单.md` | 真实比赛端到端试跑验收清单 |

另外还有：
- [`docs/agent/DevelopmentLog.md`](docs/agent/DevelopmentLog.md) — 开发者开发日志（中文）
- [`docs/上线前审计.md`](docs/上线前审计.md) — 上线前审计报告（18 条逻辑链分析）
- [`frontend/src/pages/volleyball/README.md`](frontend/src/pages/volleyball/README.md) — 排球模块专项文档（1297 行）
...
```
Scoring/
├── docs/                           # 项目文档体系（只含 .md，完整索引见 docs/agent/CLAUDE.md）
│   ├── 架构.md / 数据库.md / 业务规则.md / 类图.md / 用例.md
│   ├── 界面设计.md / 技术栈.md / 测试策略.md / 面试准备.md / 实测清单.md
│   ├── 后台管理.md / API接口.md / 上线前审计.md / 项目简介.md / 产品介绍页规划.md
│   └── agent/                      # Agent 协作文档（提示词/总结/日志，不改名）
│       ├── AGENTS.md               # Codex 协作入口
│       ├── CLAUDE.md               # AI 协作文档入口
│       └── DevelopmentLog.md       # 开发者开发日志
│
├── frontend/                       # uni-app 小程序（独立项目）
│   ├── src/
│   │   ├── pages/                  # 赛事大厅/我的/创建/记分板/赛事/排名/签名/排球
│   │   ├── components/             # 全局共享组件（MatchCard 等）
│   │   ├── composables/            # useScoreAnnouncer.js（语音播报）
│   │   ├── utils/                  # request.js 等纯逻辑工具（配套单测）
│   │   ├── store/auth.js           # 登录状态管理
│   │   ├── static/                 # logo、校徽、audio_xiaoxiao 音频
│   │   ├── pages.json              # 页面路由 + TabBar 配置（当前生效）
│   │   ├── manifest.json           # uni-app 应用配置
│   │   └── App.vue / main.js / uni.scss
│   ├── scripts/prepare-mp-weixin-env.mjs   # 本地开发环境准备
│   ├── package.json / package-lock.json    # 前端依赖（npm 命令在 frontend/ 下执行）
│   ├── vite.config.js              # Vite + API 代理配置
│   ├── vitest.config.js            # 前端单测配置
│   ├── index.html                  # H5 入口模板
│   └── dist/dev/mp-weixin/         # 微信小程序编译输出
│
├── web/                            # Web 前端
│   ├── admin-web/                  # 后台管理网页 (Vue 3 + Vite，www.eunomia.cc)
│   │   └── src/{main.js, services/api.js, views/, components/, styles.css}
│   └── product-web/                # 产品介绍静态站（纯 HTML，product.eunomia.cc）
│       ├── *.html + assets/        # 页面 + guide.css/guide-scale.js + 截图素材
│       └── PRODUCT_PAGE_PLAN.md 移入 docs/产品介绍页规划.md
│
├── backend/                        # 后端源代码 (Spring Boot)
│   ├── pom.xml
│   ├── start-local.ps1/.cmd + local-env(.example).ps1  # 本地启动/环境脚本
│   ├── src/main/java/com/scoring/backend/
│   │   ├── controller/             # REST 接口层（Auth/Tournament/Match）
│   │   ├── service/                # 业务逻辑层（接口 + 实现）
│   │   ├── engine/                 # 核心算法（Bracket/RoundRobin/ranking）
│   │   ├── domain/{entity,dto,vo}  # 数据模型
│   │   ├── mapper/  security/  common/  config/
│   ├── src/test/java/              # 单元测试与集成测试
│   ├── src/main/resources/db/migration/  # Flyway 迁移 (V1~V20)
│   └── deploy/                     # deploy-prod / rollback-prod / backup-db /
│                                   # start-prod + nginx/ + systemd/
│
├── scripts/                        # 项目脚本
│   ├── deploy/                     # 发布/部署 runbook（6 个 .ps1，见各脚本注释）
│   └── legacy/                     # 历史遗留归档（仅审计保留，见 README.md）
│
├── .env.example                    # 环境变量模板
└── .gitignore
```
...
- **真实比赛试跑**：按 [`docs/实测清单.md`](docs/实测清单.md) 完成端到端验收
