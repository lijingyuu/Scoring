...
- **字符集**: `utf8mb4`
- **引擎**: InnoDB
- **迁移工具**: Flyway（22个迁移版本，V1 ~ V22）
- **ID 策略**: MyBatis-Plus `ASSIGN_ID`（雪花算法，19位数字，**以字符串传输**；`web_login_session` 例外，用 BIGINT 自增）
- **表数量**: 19 张（`match_theme_config` 为历史残留实体/Mapper，`global_theme_config` 仅有建表脚本，无有效实体/Mapper/API；V21 `match_runtime_state` 已建表暂未接线；V22 `web_login_session` 为 PC 扫码登录临时会话）
