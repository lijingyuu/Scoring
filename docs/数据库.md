...
> 写后端 SQL 与前端条件渲染（`v-if`）时的字段/枚举权威来源；所有枚举值以此为准。
> 关联：[[架构.md]] · [[业务规则.md]]
...
- **字符集**: `utf8mb4`
- **引擎**: InnoDB
- **迁移工具**: Flyway（23个迁移版本，V1 ~ V23）
- **ID 策略**: MyBatis-Plus `ASSIGN_ID`（雪花算法，19位数字，**以字符串传输**；`web_login_session` 例外，用 BIGINT 自增）
- **表数量**: 20 张（V23 新增 `tournament_division` 组别表，赛事与比赛之间的层级；`match_theme_config` 为历史残留实体/Mapper，`global_theme_config` 仅有建表脚本，无有效实体/Mapper/API；V21 `match_runtime_state` 已建表暂未接线；V22 `web_login_session` 为 PC 扫码登录临时会话）
...
> **V23 起组别层级**：`tournament_type / group_size / knockout_slots / knockout_rounds / qualifiers_per_group / round_robin_rounds / round_rule_enabled / current_stage / knockout_generated / best_of / games_to_win / points_to_win / deciding_points_to_win / enable_deuce / cap_point / third_place_*` 等赛制与规则列已**下沉到 `tournament_division`**（见 §2.19）。本表这些列仅作占位：创建时写入第 1 个组别的值，单组别赛事在状态变更时镜像写入（回滚保险），业务读写一律以组别为准。`status` 仍为本表权威字段（聚合状态：全部组别完赛才置 2）。
...
### 2.18 `web_login_session` — PC 网页扫码登录临时会话（V22 新增）

PC 端（admin-web）扫码登录的票据状态机，3 分钟有效期，票据即小程序码 scene（32 位 hex）。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `id` | BIGINT | PK, 自增 | |
| `ticket` | VARCHAR(32) | NOT NULL, UNIQUE | 32位hex票据（去横杠UUID），即小程序码 scene |
| `status` | VARCHAR(16) | NOT NULL | CREATED 已出码 / SCANNED 已扫码 / CONFIRMED 已确认 / CONSUMED 已取走 token |
| `user_id` | VARCHAR(32) | | 扫码/确认用户 |
| `expire_time` | DATETIME | NOT NULL | 过期时间（出码 + 3 分钟），有索引 |
| `create_time` | DATETIME | | |
| `update_time` | DATETIME | | ON UPDATE |
...
- token 不落库：CONFIRMED 只存 `user_id`，PC 首次轮询取走时现签 JWT 并原子转 `CONSUMED`（一次性下发防重放）
- 状态转移全部用条件 UPDATE 原子完成（与 `match_record` 执裁锁同风格）
- 懒清理：出码时顺带删除过期超 24h 的行，防表膨胀
...
### 2.19 `tournament_division` — 组别（V23 新增）

> 赛事与比赛之间的层级。**每个赛事必有 ≥1 个组别**（V23 迁移为存量赛事回填一个"默认组别"，id = 赛事id + 'D01'）。一个组别 = 一个完整子赛事：赛制、计分规则、轮次规则键、排名配置键、进度状态均以组别为单位。多组别仅支持羽毛球个人赛（服务层校验）。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `id` | VARCHAR(32) | PK | 运行时雪花 ID；存量回填为 `赛事id+'D01'` |
| `tournament_id` | VARCHAR(32) | NOT NULL, IDX | 所属赛事 |
| `name` | VARCHAR(64) | NOT NULL | 组别名（如 男单组），赛事内唯一 `uk_division_tournament_name` |
| `sort_order` | INT | DEFAULT 0 | 展示顺序（= 创建时数组下标）；兼容层取第 1 个 |
| `status` | TINYINT | DEFAULT 0 | 0-未开始 1-进行中 2-已结束（语义同赛事 status，组别级） |
| `participant_type` | TINYINT | DEFAULT 0 | 预留，v1 恒 0（个人） |
| `tournament_type` | TINYINT | DEFAULT 0 | 同 §3.3（自 tournament 下沉） |
| `group_size` / `knockout_slots` / `knockout_rounds` / `qualifiers_per_group` / `round_robin_rounds` | | | 自 tournament 下沉 |
| `current_stage` | TINYINT | DEFAULT 1 | 同 §3.4（自 tournament 下沉） |
| `knockout_generated` | TINYINT(1) | DEFAULT 1 | 自 tournament 下沉 |
| `best_of` / `games_to_win` / `points_to_win` / `deciding_points_to_win` / `enable_deuce` / `cap_point` | | | 计分规则（自 tournament 下沉） |
| `round_rule_enabled` | TINYINT(1) | DEFAULT 0 | 是否启用赛段规则（自 tournament 下沉） |
| `third_place_*`（7 列） | | | 三四名决赛规则（自 tournament 下沉） |
| `create_time` / `update_time` | DATETIME | | |

**V23 同步变更的表**（均加 `division_id VARCHAR(32) NOT NULL` + 索引，`tournament_id` 保留用于权限/团体赛路径）：

- `player`：选手归属组别（`idx_player_division_id`）
- `match_record`：比赛归属组别（`idx_match_division_id`）
- `tournament_round_rule`：唯一键 `(tournament_id, stage_type, round_num)` → `(division_id, stage_type, round_num)`
- `tournament_ranking_config`：唯一键 `(tournament_id)` → `(division_id)`（每组别一份独立配置）
- `tournament_qualification_override`：两个唯一键从 tournament_id 改锚 division_id

**状态语义**：`tournament.status` 为聚合状态——任一组别开赛 → 1；**全部**组别 status=2 才置 2。完赛判定并发安全：按 id 升序对所有组别行 FOR UPDATE 后聚合（MatchSettlementService.finishDivisionIfReady）。
