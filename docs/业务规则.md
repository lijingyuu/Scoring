...
> 固化最烧脑的体育规则：赛制流转、计分、排名/晋级、排球专项。算法约束以此为准。
> 关联：[[数据库.md]] · [[架构.md]]
...
### 1.5 分组抽签（assignGroups）

```
创建分组赛 (tournamentType=1) 时执行，个人赛/羽毛球团体/排球共用:

Step 1 · 拆分名单
  - 刻意设置了种子(seedRank)的选手 → 按种子序升序
  - 未设置种子的选手 → Collections.shuffle() 随机打乱
    (禁止任何确定性兕底，如按名字排序)

Step 2 · 蛇形分发
  拼接[种子在前，随机选手在后]后按行蛇形分配:
  第 1 行: 组1→组2→…→组N (正序)
  第 2 行: 组N→…→组2→组1 (逆序)
  交替往复，groupPosition 记录组内位次

效果:
  - 全员无种子 → 整份名单绝对随机分组
  - 有 k 个种子(k≤组数) → 种子互不同组，其余选手随机
```
...
创建者可将已结束的赛事（status=2）归档：`PUT /tournaments/{id}/archive` 置 `archived=1`，从大厅与"我的创建"隐藏、移至"我的 → 归档"；`PUT /tournaments/{id}/unarchive` 恢复。归档是**强约束**——`archived=true` 压过创建者/裁判操作权限（`canOperateMatches` 返回 false），且对阵图/小组赛关闭编辑、只读；创建者仍可在归档页查看并取消归档。
...
后端逻辑（MatchServiceImpl.canOperateMatch()，裁判授权判定同款逻辑收敛于 TournamentRefereeService）：
...
三种身份入口（小程序微信登录 / Web 注册 / Web 密码登录，接口见 [[API接口.md]]）最终都签发同一种 JWT；后续比赛操作权限统一走 `AuthInterceptor → AuthContext → AuthGuard`。
...
- **续期**：`POST /api/v1/matches/{id}/heartbeat`，前端每 15s 一次；仅持有者可续期，把过期时间顺延 75s（`MatchLockService.MATCH_LOCK_SECONDS`）。
