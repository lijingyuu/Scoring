...
- 单项状态: ✅已完赛(绿色) / ⏳进行中(橙色) / ⬜待赛
- 一方达到 3 胜时显示"是否直接结算"提示弹窗，确认后回到上一层赛程页
- 子比赛返回后自动刷新 → 若父比赛已结束自动跳回对阵图

### 9.2 团体赛阵容编辑 (`team-lineup.vue`)
...
- 苏杯模式: 5 项固定编码（MS/WS/MD/WD/XD）
- 接力模式: N 段（R1..RN），闭环链条验证
- 已选队员灰色/不可再选（防重复）
- 保存后优先返回页面栈中已有的 team-match/team-relay 页，避免重复堆栈

### 9.3 接力赛记分板 (`team-relay.vue`)
...
- 总分制（不分局），段目标动态更新
- 到达段目标 → 弹窗提示"进入下一段"
- 累计分同步到后端（relaySegmentScores）
- 换边不影响逻辑分

### 9.4 团体赛战报 (`team-record.vue`)
...
- 纸质战报风格，区别于深色操作页
- 只展示苏迪曼杯式 5 项，不用于接力赛记分
- 签名区使用 canvas，确认后显示签名图片

### 9.5 我的列表 / 归档 (`mine-list.vue`, `archived.vue`)
...
- `mine-list.vue` 通过 `?type=` 参数区分收藏/创建
- `archived.vue` 显示已归档赛事，支持取消归档
- 归档赛事有特殊样式标识
...
在非 Pad 布局下，首页结果面板展示半透明校徽水印（`NJUschoolbadge.png`），Pad 布局下隐藏以避免视觉干扰。

### 10.2 Pad/Phone 自适应

```js
updateDeviceLayout():
  isPadLayout = Math.min(windowWidth, windowHeight) >= 720
```

- Phone: 显示校徽水印
- Pad: 隐藏水印，利用宽屏空间展示更多赛事信息

### 10.3 搜索交互

- 首页默认不展示赛事列表（需输入关键词搜索）
- 输入关键词后实时请求 `GET /tournaments?keyword=`
- 清空关键词恢复空列表
...
| 8 | 团体赛页面在页面栈中跳转混乱 | 使用 `tournament-navigation.js` 统一导航，先遍历页面栈再用 redirectTo 兜底 |
| 9 | 团体赛名单页保存后重复堆叠 team-match/team-relay | 保存后优先回退到页面栈中已有团体赛页，找不到时再 redirectTo |
