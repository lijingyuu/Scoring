...
仓库内除 uni-app `frontend/src/` 外还有一个独立前端 `web/admin-web/`（www.eunomia.cc 后台管理网页），**不参与** uni-app 编译，单独构建部署：
...
- 详细功能与路线见 [[后台管理.md]]；部署 runbook 见 `scripts/deploy/`
...
> 完整接口文档见 [[API接口.md]]，共 42 个有效接口（不含已废弃的主题配置接口）
