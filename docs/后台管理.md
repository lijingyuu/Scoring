...
> **用途**: 记录 `web/admin-web/` 子项目的定位、结构、已完成功能与后续路线。
> **关联**: [[架构.md]] · [[API接口.md]] · [[API接口.md|API 契约]]
> **部署运维**: 见 `scripts/deploy/SKILL.md`（部署 runbook，不在本文档重复）
...
| `product.eunomia.cc` | 产品介绍静态页 | `web/product-web/` 纯 HTML |
...
- **完全复用现有接口**：不新增任何后端接口，与小程序共用同一套 `/api/v1` 契约（见 [docs/API接口.md](docs/API接口.md)）。
...
启动命令（`frontend/package.json` 已封装，需在 `frontend/` 下执行）：

```bash
cd frontend
npm run admin:dev     # 开发（端口 5173，代理 /api）
npm run admin:build   # 构建 → web/admin-web/dist/
```
