...
- `EXPIRED`：二维码遮罩「点击刷新」；出码失败（429 限流/微信故障）同样给遮罩重试；
  `CONSUMED`（拿到 token 的那次响应在网络上丢失，token 一次性不可恢复）也立即转遮罩引导刷新，不会卡在「已扫码」等待过期
- 轮询定时器有代数守卫：快速切 tab / 卸载时在途的旧出码与旧轮询全部作废，不产生孤儿 interval

**出码模式环境变量**（服务器 `/opt/scoring/backend/deploy/prod-env.sh`，改后需 `systemctl restart scoring-backend`）：

| 变量 | 默认（正式） | 灰度值 | 说明 |
|------|------|------|------|
| `WECHAT_QR_CHECK_PATH` | 不设（即 true） | `false` | true 时要求 `pages/auth/pc-confirm` 已随正式版发布，否则出码报 41030 |
| `WECHAT_QR_ENV_VERSION` | 不设（即 release） | `trial` | 指向体验版，仅体验成员可扫码打开 |

灰度已用于 2026-09-10 的真机验证并已切回正式模式；正式版发布前出码接口报 41030 属预期。
