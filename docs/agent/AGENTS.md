...
| 文档 | 用途 |
|------|------|
| `CLAUDE.md` | 项目总览、启动命令、关键约定（同在 docs/agent/） |
| `docs/架构.md` | 前后端分层、目录职责、路由和接口速查 |
| `docs/类图.md` | 核心领域、赛程、记分、团体赛、排球状态类图 |
| `docs/用例.md` | 核心功能用例、参与者、主流程和验收核对点 |
| `docs/数据库.md` | 17 张表结构、状态码和枚举 |
| `docs/API接口.md` | 当前有效 REST API 契约 |
| `docs/业务规则.md` | 赛制、计分、晋级、排球专项规则 |
| `docs/界面设计.md` | 前端视觉与交互约定 |
| `docs/技术栈.md` | 项目用到哪些技术点、在本项目里具体怎么落地 |
| `docs/面试准备.md` | 项目亮点、踩坑复盘、设计决策答辩稿 |
| `docs/实测清单.md` | 真实比赛端到端试跑验收清单 |
| `docs/产品介绍页规划.md` | 产品介绍网页的信息架构、模块规划、展示卖点和视觉偏好 |
...
- 修改 `web/product-web/` 产品介绍网页时，凡涉及顶部模块、页面结构、核心卖点、视觉方向或后续可能复用的重要判断，都同步记录到 `docs/产品介绍页规划.md`。
- 后端改动遵循 Controller → Service → Engine/Mapper 分层，Controller 不写业务逻辑。
- 前端改动遵循 `frontend/src/pages/` 路由页、`frontend/src/components/` 全局组件、页面专属组件就近放置。
- 在 Codex Windows 沙箱中运行前端 npm 脚本时，固定使用 `npm.cmd`，例如 `npm.cmd test`、`npm.cmd run build:h5`，避免 PowerShell 对 `npm.ps1` 的执行策略拦截。
- 测试遇到沙箱/权限拦截时，先判断是否已经足够说明问题：前端 `npm.cmd test` 或构建如果被执行策略、文件锁、`frontend/dist/build` 权限拦截，不反复申请提升权限或清理重跑；在最终答复中明确写出“未验证，原因是 xxx”即可。后端 Maven 若因 `backend/target` 写入被拒绝，只在任务确实需要后端验证时申请一次提升权限，用户中断或拒绝后立即停止重试并报告。
...
- 前端 H5 构建如果报 `EPERM: operation not permitted, unlink/mkdir 'frontend\dist\build\h5...'`，优先判断为旧构建产物或 Windows 沙箱文件权限问题，不要反复直接重跑；先清理生成目录 `Remove-Item -LiteralPath 'frontend\dist\build' -Recurse -Force`，必要时只为清理/重建生成目录申请一次提升权限，然后再跑 `npm.cmd run build:h5`。
- 小幅前端展示微调（只改布局、颜色、字号、间距等样式，不改逻辑/接口/路由）完成后默认不跑完整编译或测试；只做必要的代码核对，除非用户明确要求验证。
- 连续做多轮小 UI 调整时，先用 `rg` 和局部文件读取确认文案/样式是否落点正确，把多次微调合并后最后只跑一次必要构建；如果构建只因 `frontend/dist/build` 产物权限失败，按上一条清理产物后重跑，不把它当作源码问题继续排查。
...
- 排球记分核心逻辑在 `frontend/src/pages/volleyball/composables/useScoreboard.js`，Phone/Pad 组件只处理 UI 差异。
