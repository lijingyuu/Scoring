...
export function useKnockoutBracketViewport(layout, selector = '.bracket-viewport') {
  ...
  async function fitToOverview(retryCount = 3) {
    ...
    const nextScale = calculateOverviewScale(size, viewportSize)
    scale.value = nextScale
    overviewScale.value = nextScale
    maxScale.value = Math.max(KNOCKOUT_BRACKET_VIEWPORT.maxScale, nextScale)
    x.value = 0
    y.value = 0
  ...
  async function resetView() {
    const viewportSize = await measureViewport()
    const size = layoutSize()
    const nextScale = clampBracketScaleWithin(1, overviewScale.value, maxScale.value)
    const nextPosition = centerBracketPosition(size, viewportSize, nextScale)
    scale.value = nextScale
    x.value = nextScale <= overviewScale.value + 0.001 ? 0 : nextPosition.x
    y.value = isVerticalLocked(nextScale, viewportSize) ? 0 : nextPosition.y
    if (nextScale <= overviewScale.value + 0.001 || isVerticalLocked(nextScale, viewportSize)) {
      forcePositionSync(nextScale <= overviewScale.value + 0.001, true)
    }
  }
